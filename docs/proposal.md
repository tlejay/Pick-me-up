...
```mermaid
graph TD
    A[Open App] --> B[Browse Coffee Shops]
    B --> C{Select Shop}
    C --> D[View Menu]
    D --> E[Add Items to Cart]
    E --> F[Customize Order]
    F --> G[Review Order]
    G --> H[Select Pick-up Time]
    H --> I[Pay via PromptPay]
    I --> J[Receive Confirmation]
    J --> K[Track Order Status]
    K --> L{Order Ready?}
    L -->|Preparing| K
    L -->|Ready| M[Go to Shop]
    M --> N[Pick up Order]
    N --> O[Leave Review]
...
```mermaid
graph TD
    A[Receive New Order] --> B[Notification Alert]
    ...
    I --> J[Customer Arrives]
    J --> K[Verify Order]
    K --> L[Hand over Order]
    L --> M[Mark as Completed]
    M --> N[View in Sales Report]

    E --> O[Customer Notified]
